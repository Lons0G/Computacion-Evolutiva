...
std::shared_ptr<Individual> Ruleta::select(const Population& population) const {
...
double Ruleta::calculateTotalFitness(const Population& population) const {
...
Torneo::Torneo(int tournamentSize) : _tournamentSize(tournamentSize) {
    if (tournamentSize < 1) {
        throw std::invalid_argument("Tournament size must be at least 1");
    }
}

std::shared_ptr<Individual> Torneo::select(const Population& population) const {
    if (population.size() == 0) {
        throw std::runtime_error("Cannot select from empty population");
    }
...
std::vector<std::shared_ptr<Individual>> Universal_Estocastica::selectMany(
    ...
    // RETORNAR VECTORES CON LOS INDIVIDUOS DE LOS SELECCIONADOS
    return selected;
}
...
std::unique_ptr<ISelectionStrategy> SelectionFactory::create(Type type, int tournamentSize) {
    switch (type) {
        case Type::RULETA:
            return std::make_unique<Ruleta>();
        case Type::TORNEO:
            return std::make_unique<Torneo>(tournamentSize);
        default:
